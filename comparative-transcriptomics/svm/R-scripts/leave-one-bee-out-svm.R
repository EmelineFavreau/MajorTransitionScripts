...
# Our data are read counts provided by RSEM for 3 bees, for 5787 orthogroups.
...
# focus_species    <- "Megalopta_genalis"
focus_species    <- args[1]
# training_species <- c("Ceratina_australensis", "Ceratina_calcarata")
training_species <- c(args[-c(1,2)])
# experiment_name <- "5787_orthogroups"
...
# 5787 obs. of  29 variables
counts_clean <- read.csv("input/readcounts_5787orthogroups_beeonly.txt",
...
# subset for only bees
pheno_data <- pheno_data %>% filter(Species %in% c("Ceratina australensis",
                                                   "Ceratina calcarata",
                                                   "Megalopta genalis"))
...
while(nfeatures > nfeatures_target){
  ...
  # extract feature weights (parameters of hyperplane: coeffs are weight for  SV: Support Vector)
  ...
  # vector of probability of being reproductive for each feature being dropped
