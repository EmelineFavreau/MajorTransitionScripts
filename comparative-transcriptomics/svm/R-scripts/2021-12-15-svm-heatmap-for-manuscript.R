# Heatmap of SVM genes for all species for manuscripts
# SVM = Support Vector Machine (species-normalised)

# import data 
# columns: nfeatures, orthogroup, followed by sample names
# rows: for each orthogroup, the sample has one probability value (0 = worker, 1 = queen)
# table is ordered from the least important feature (nfeature = 3718)
# to the most important feature (100 and lower)

Caus_table <- 
  read.csv("../result/species-normalised/regression-on-all/Caus/Ceratina_australensis_Caus_all_regr_norm_prediction_table",
                       sep="", stringsAsFactors=FALSE)

Ccal_table <- 
  read.csv("../result/species-normalised/regression-on-all/Ccal/Ceratina_calcarata_Ccal_all_regr_norm_prediction_table",
                       sep="", stringsAsFactors=FALSE)

Lfla_table <- 
  read.csv("../result/species-normalised/regression-on-all/Lfla/Liostenogaster_flavolineata_Lfla_all_regr_norm_prediction_table",
                       sep="", stringsAsFactors=FALSE)

Mgen_table <- 
  read.csv("../result/species-normalised/regression-on-all/Mgen/Megalopta_genalis_Mgen_all_regr_norm_prediction_table",
                       sep="", stringsAsFactors=FALSE)

Pcan_table <- 
  read.csv("../result/species-normalised/regression-on-all/Pcan/Polistes_canadensis_Pcan_all_regr_norm_prediction_table",
           sep="", stringsAsFactors=FALSE)

Pdom_table <- 
  read.csv("../result/species-normalised/regression-on-all/Pdom/Polistes_dominula_Pdom_all_regr_norm_prediction_table",
           sep="", stringsAsFactors=FALSE)


# these are the raw reads from the SVM analysis
readcounts_3718orthogroups_6species <- 
  read.csv("../input/readcounts_3718orthogroups_6species.txt",
                                                sep="",
...
# combine info (colnames nfeatures orthogroup)
...
# number of features kept (from ST14)
Caus <- 636
Ccal <- 533
Lfla <- 399
Mgen <- 494
...
# dendogram of species do not cluster by clade
# (Pdom (Mgen (Caus (Lfla (Pcan, Ccal)))))
plot(hc.cols)
...
# species are a bit more clustered by calde
# ( Lfla(Pdom (Mgen ( Pcan(Caus, Ccal)))))
# (wasp(wasp(bee(wasp(bee, bee)))))
plot(hc.cols)
...
# draw heatmap with dendrogram of orthologs
# this figure goes in manuscript
pdf("all-predictor-genes-species-normalised-heatmap.pdf")
