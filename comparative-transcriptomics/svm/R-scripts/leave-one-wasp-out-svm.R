...
# Our data are read counts provided by RSEM for 3 wasps, for 6983 orthogroups.
...
# training_species <- c("Polistes_dominula", "Polistes_canadensis")
training_species <- c(args[-c(1,2)])
# experiment_name <- "6983_orthogroups"
...
# 6983 obs. of  54 variables
counts_clean <- read.csv("input/readcounts_6983orthogroups_wasponly.txt",
...
# subset for only wasps
pheno_data <- pheno_data %>% filter(Species %in% c("Polistes dominula",
                                                   "Polistes canadensis",
