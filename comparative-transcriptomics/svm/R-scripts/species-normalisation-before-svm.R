...
# other useful input files: list of orthogrop/species gene hash tables,
# sample and phenotype info

# running also a species normalisation

library("tidyverse")

# load all species' read count data
Ceratina_australensis_rawReadCounts <- 
  read.table("../input/Ceratina-australensis_raw-read-counts",
  header = TRUE, stringsAsFactors = FALSE)

Ceratina_calcarata_rawReadCounts <- 
  read.table("../input/Ceratina-calcarata_raw-read-counts",
  header = TRUE, stringsAsFactors = FALSE)

Megalopta_genalis_rawReadCounts <- 
  read.table("../input/Megalopta-genalis_raw-read-counts",
  header = TRUE, stringsAsFactors = FALSE)

Liostenogaster_flavolineata_rawReadCounts <- 
  read.table("../input/Liostenogaster-flavolineatas_raw-read-counts",
  header = TRUE, stringsAsFactors = FALSE)

Polistes_canadensis_rawReadCounts <- 
  read.table("../input/Polistes-canadensis_raw-read-counts",
  header = TRUE, stringsAsFactors = FALSE)

Polistes_dominula_rawReadCounts <- 
  read.table("../input/Polistes-dominula_raw-read-counts",
  header = TRUE, stringsAsFactors = FALSE)

# load all species' orthogroup data
Ceratina_australensis_orthogroupsList <- 
  read.table("../input/Ceratina_australensis_3718_gene_orthogroups_list",
  header = FALSE, stringsAsFactors = FALSE)

Ceratina_calcarata_orthogroupsList <- 
  read.table("../input/Ceratina_calcarata_3718_gene_orthogroups_list",
  header = FALSE, stringsAsFactors = FALSE)

Megalopta_genalis_orthogroupsList <- 
  read.table("../input/Megalopta_genalis_3718_gene_orthogroups_list",
  header = FALSE, stringsAsFactors = FALSE)

Liostenogaster_flavolineata_orthogroupsList <- 
  read.table("../input/Liostenogaster_flavolineata_3718_gene_orthogroups_list",
  header = FALSE, stringsAsFactors = FALSE)

Polistes_canadensis_orthogroupsList <- 
  read.table("../input/Polistes_canadensis_3718_gene_orthogroups_list",
  header = FALSE, stringsAsFactors = FALSE)

Polistes_dominula_orthogroupsList <- 
  read.table("../input/Polistes_dominula_3718_gene_orthogroups_list",
  header = FALSE, stringsAsFactors = FALSE)
...
speciesPhenoSampleName$SampleName <- 
  gsub(pattern = "$", replacement = "_1Aligned.sortedByCoord.out.bam",
...
subsetrawReadCounts <- rawReadCounts %>% 
  select(Geneid, all_of(speciesSamplesdf$SampleName)) %>% 
  filter(Geneid %in% orthogroupsList$V2)


# add the orthogroups column
subsetrawReadCounts$orthogroup <- 
  orthogroupsList$V1[match(subsetrawReadCounts$Geneid, orthogroupsList$V2)]


# set number of samples
sample_num <- nrow(speciesSamplesdf)

# run the species normalisation
for(i in 1:length(subsetrawReadCounts$orthogroup)){
  orthogroup_read_count_vec <- as.numeric(subsetrawReadCounts[i, 2:(sample_num+1)])
  orthogroup_mean <- mean(orthogroup_read_count_vec)
  species_normalised_read_count_vec <- 
    (orthogroup_read_count_vec-orthogroup_mean)/orthogroup_mean
  subsetrawReadCounts[i, 2:(sample_num+1)] <- species_normalised_read_count_vec
...
for(this_orthogroup in subsetrawReadCounts$orthogroup){
  for(this_sample in speciesSamplesdf$SampleName){
    input_file_for_svm[input_file_for_svm$orthogroup == this_orthogroup,
      colnames(input_file_for_svm) == this_sample] <- 
      subsetrawReadCounts[subsetrawReadCounts$orthogroup == this_orthogroup,
...
# check result (plot before and after)             
# str(input_file_for_svm)
write.table(file = "../input/readcounts_3718orthogroups_6species-species_normalised.txt",
