#---
#title: "SVM test on Ceratina calcarata, by training on wasp only"
...
this_traindata <- pheno_data %>% 
  filter(Species %in% c("Polistes canadensis",
    "Polistes dominula", "Liostenogaster flavolineata")) %>% 
  select(SampleName, Phenotype)

# make a test data: sample labels and reproductives
# just calcarata
this_testdata <- pheno_data %>% 
  filter(Species == "Ceratina calcarata") %>% 
  select(SampleName, Phenotype)

# create filename for gene list in full model
FileName <- "Ceratina_calcarata_svm_agst_wasp_only_predicted_gene_list"
plotFileName <- "Ceratina_calcarata_svm_agst_wasp_only_plot.pdf"
ErrorRateTable <- "Ceratina_calcarata_svm_agst_wasp_only_error_rates"
#### Define SVM function
