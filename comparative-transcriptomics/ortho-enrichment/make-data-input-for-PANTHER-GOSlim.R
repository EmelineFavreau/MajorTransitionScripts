...
# specifically, we need the Dmel genes id associated with DEGs proteins
...
for(myspecies in species_vec){  
  ...
  # obtain Dmel protein that are DEGs in species
  Dmel_protein_DEGs <- raw_blast_results$sseqid[match(raw_selectionGenes$gene,
                                                      raw_blast_results$qseqid)]
  Dmel_protein_DEGs <- Dmel_protein_DEGs[!is.na(Dmel_protein_DEGs)]
  
  # obtain Dmel genes that are DEGs in species
  Dmel_genes_DEGs <- gene2GoSlim_raw$flybase_gene_id[match(Dmel_protein_DEGs,
                                        gene2GoSlim_raw$flybase_translation_id)]
  
  # vec of file name
  fileName <- paste(myspecies, "Dmel_genes_DEGs_SlimPanther.txt", sep = "_")
  
  # save it to import into Galaxy GoSlimmer
  write.table(Dmel_genes_DEGs,
              file      = fileName,
              row.names = FALSE,
              col.names = FALSE,
              quote     = FALSE,
