...
# vector of all datasubsets available
datasubset_vec <- c("R", "NR")

# loop over the species
for(myspecies in species_vec){
    # loop over the datasubset
    for(datasub in datasubset_vec ){
      # loop over the categories
        for(gocat in goCategory_vec){
            ...
            this_species     <- args[1]
            this_data_subset <- args[2]
            this_goCategory  <- args[3]
      ...
      print(paste(datasub, "is done", sep = " "))
