...
vst.list <- list(Caus.vst, Ccal.vst, Mgen.vst, Pcan.vst, Pdom.vst, Lfla.vst)
## Then, to access the data, use the assay() function:
vst.assay <- list(assay(Caus.vst), assay(Ccal.vst), assay(Mgen.vst), assay(Pcan.vst), assay(Pdom.vst), assay(Lfla.vst))
```
...
for(i in 1:length(species)) {
    ...
    # pca + geom_label(aes(label = colnames(vst.list[[i]]))) + 
      ...
      pca <- pca + ggrepel::geom_text_repel(aes(label = colnames(vst.list[[i]])), size = 2.25, position = nudge) +
            geom_point(size = 1.5) +             
            scale_shape_manual(values = c("NR" = 1, "R" = 2)) +
            scale_color_manual(values = c("NR" = "cornflower blue", "R" = "darkgoldenrod2")) +
            theme_classic() + 
            xlim(-45, 45) +
            ylim(-35,35) +
