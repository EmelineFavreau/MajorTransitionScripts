---
title: "DESeq2 Analyses: <i>Ceratina calcarata</i>"
author: "KS Geist"
...
## Import data
hymn <- read.table("Rehan2018_calc_reads_all.csv", header = T, sep = ",")
#names(hymn)
...
## At the moment, we have more samples than desired for repro vs. nonrepro. So, let's just grab the columns we want:
hymn <- hymn[,c("F91_AB", "F99_AB", "F95_AB", "K70.FD", "W46.FD", "K86.FD")]
names(hymn)
...
head(hymn)
```
**Note: AB = Active Brood vs. FD = Foraging Daughter. Shell & Rehan (2019) got 488 DEGs with DESeq2 on this comparison.**
...
**Note, for comparison with other upstream methods, 33% of genes have zero counts across all samples. Total read count: 73,336,820 vs only 20,702,772 in the newer pipeline.**
...
```{r}
id <- c("F91_AB", "F99_AB", "F95_AB", "K70.FD", "W46.FD", "K86.FD")
phenotype <- c(rep("R", 3), rep("NR", 3))
...
**This is what we are seeing here.** A variance-stabilizing transformation will help to resolve this issue.**We see a similar pattern as we do in the data from the newer analysis**
...
plotDispEsts(dds)
```
It looks a little less nice than the newer analysis but it's not horrible. Post-shrinkage (blue), everything fits nicely except the most variable genes. There is a gap / pattern in the data for a handful of genes. 
...
```{r}
#save(vsdata1, file = "Ccalcarata_VarStab.RData")
write.csv(file = "Ccalcarata_VarStab.csv", assay(vsdata1), row.names = T, quote = F)
```
...
```
Some Rs and non are quite dissimilar, but the reproductive phenotypes don't cluster distinctly. The NR phenotypes sort of do. **This is the same pattern we see with the newer analysis too**
...
pheatmap(pwCorr)
```
There is better clustering than we've seen in some of the other species despite the small sample sizes except that one mother (reproductive) who is just different from everybody else... **Again, the same pattern we see as in the newer analysis, so that's confirmatory**
...
DEGenumerator <- function(dds) {
  for (i in 1:length(lfc.thresholds)) {
    ...
    ## Put in a dataframe:
    LFC.df <- rbind(LFC.df, c(lfc.thresholds[i], linear.changes[i], total, up, down, "Ccalcarata"))
  }
...
```
Note: This time we get 56 total DEGs vs. 33 DEGs. With such a small number of DEGs, this is a big deal.
...
```
According to Shell & Rehan (2019), the DESeq2 results were *post hoc* filtered using a false discovery rate (FDR) cutoff of FDR < 0.05. 

Yes, now I get the results that are similar to theirs: 479 here vs. 488 in Shell & Rehan (2019).

One thing I am noticing is that I get the same number of DGEs with just p-value or the combined p-value & LFC of 1.5 filtering. I have increased the LFC threshold to 1 (for a linear fold change of 2). I am also printing summary results of the LFC for the method where I filter on p-value alone.
...
plotCounts(dds, gene="Ccalc.v2_003401", intgroup="phenotype")
plotCounts(dds, gene="Ccalc.v2_004787", intgroup="phenotype")
plotCounts(dds, gene="Ccalc.v2_005563", intgroup="phenotype")
```
**This might be important. Firstly, the identity of the top DEGs change: but for two of them, their numbers shift by 1 (0030400 in the new analysis, 0030401 here; 004788 in the new analysis, 004787 here -- so shifting in two directions, so it's possible they are the same genes and it's a ref genome version discrepancy?) However, we see a very differe geneID for the third: 011506 in the new analysis, and 005563 here.
...
# top20_sigDGE_genes <- res_tableOE_tb %>% 
#         arrange(padj) %>% 	#Arrange rows by padj values
#         pull(gene) %>% 		#Extract character vector of ordered genes
#         head(n=20) 		#Extract the first 20 genes
# 
# top20_sigDGE_genes <- as.data.frame(top20_sigDGE_genes)
# 
# ## normalized counts for top 20 significant genes
# top20_sigDGE_norm <- as.data.frame(normalized_counts) %>% filter(gene %in% top20_sigDGE_genes)
# 
# # Gathering the columns to have normalized counts to a single column
# gathered_top20_sigDGE <- top20_sigDGE_norm %>% 
#   gather(colnames(top20_sigDGE_norm)[2:9], key = "samplename", value = "normalized_counts")
# 
# ## check the column header in the "gathered" data frame
# View(gathered_top20_sigDGE)
# 
...
# gathered_top20_sigDGE <- inner_join(mov10_meta, gathered_top20_sigDGE)
# 
# ## plot using ggplot2
# ggplot(gathered_top20_sigDGE) +
...
#               filter(gene %in% sigDGE$gene) %>% 
# 	            data.frame() %>%
# 	            column_to_rownames(var = "gene")
# 
# normalized_counts <- as.data.frame(normalized_counts)
# is.data.frame(sigDGE)
```
