---
title: "DESeq2 Analyses: <i>Liostenogaster flavolineata</i>"
author: "KS Geist"
...
## Import data
hymn <- read.table("Lflavolineata_merged_gene_counts.txt", header = T, sep = "\t")
...
#names(hymn)
colnames(hymn) <- c("SocF7", "SocR8", "SocR1", "SocR4", "SocR2", "SocF12", "SocF14", "SocR10", "SocR3", "SocR6", "SocF4", "SocF6", "SocF9", "SocF10", "SocR5", "SocF5", "SocF11", "SocR7", "SocF8")

## Next, let's reorder the headers by phenotype, 9 Queens, 10 workers:
hymn <- hymn[, c("SocR10", "SocR1", "SocR2", "SocR3", "SocR4", "SocR5", "SocR6", "SocR7", "SocR8", "SocF10", "SocF11", "SocF12", "SocF14", "SocF4", "SocF5", "SocF6", "SocF7", "SocF8", "SocF9")]
...
## Read count by phenotype (first 9 are R's, second 10 are NR's)
sum(hymn[,c(1:9)])
sum(hymn[,c(10:19)])
...
```{r}
id <- c("SocR10", "SocR1", "SocR2", "SocR3", "SocR4", "SocR5", "SocR6", "SocR7", "SocR8", "SocF10", "SocF11", "SocF12", "SocF14", "SocF4", "SocF5", "SocF6", "SocF7", "SocF8", "SocF9")
phenotype <- c(rep("R",9), rep("NR", 10))
...
```{r}
#save(vsdata1, file = "Lflavolineata_VarStab.RData")
write.csv(file = "Lflavolineata_VarStab.csv", assay(vsdata1), row.names = T, quote = F)
```
...
```
Some Rs and non are quite dissimilar, but the phenotypes don't cluster at all!
...
pheatmap(pwCorr)
```
There isn't any clustering, really.
...
DEGenumerator <- function(dds) {
  for (i in 1:length(lfc.thresholds)) {
    ...
    ## Put in a dataframe:
    LFC.df <- rbind(LFC.df, c(lfc.thresholds[i], linear.changes[i], total, up, down, "Lflavolineata"))
  }
...
plotCounts(dds, gene="Lflavo2a009580", intgroup="phenotype")
plotCounts(dds, gene="Lflavo2a013648", intgroup="phenotype")
plotCounts(dds, gene="Lflavo2a009221", intgroup="phenotype")
```
