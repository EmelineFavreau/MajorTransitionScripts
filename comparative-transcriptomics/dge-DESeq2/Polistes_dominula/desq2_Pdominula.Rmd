---
title: "DESeq2 Analyses: <i>Polistes Dominula</i>"
author: "KS Geist"
...
## Import data
hymn <- read.table("Pdominula_merged_gene_counts.txt", header = T, sep = "\t")
...
#names(hymn)
colnames(hymn) <- c("W54ORYL", "W46ORYL", "W09RDOR", "F45OR", "F03BL", "W13YLSL", "F67YLBL", "W58BLWH","W08RDOR", "F47RD", "F42OR", "W28ORYL", "W49RDYL", "W40GROR", "W56YLBL", "F38BL", "F48SL", "F51RD", "F71BL",  "F69OR", "W70RDYL", "F14GR", "W01RDYL", "F65ORWH")

## Next, let's reorder the headers by phenotype, 12 Queens, 12 workers:
hymn <- hymn[, c("F45OR", "F03BL", "F67YLBL", "F47RD", "F42OR", "F38BL", "F48SL", "F51RD", "F71BL",  "F69OR", "F14GR","F65ORWH","W54ORYL", "W46ORYL", "W09RDOR", "W13YLSL",  "W58BLWH","W08RDOR", "W28ORYL", "W49RDYL", "W40GROR", "W56YLBL", "W70RDYL","W01RDYL")]
...
## Read count by phenotype (first 4 are R's, second 6 are NR's)
sum(hymn[,c(1:4)])
sum(hymn[,c(5:10)])
...
```{r}
id <- c("F45OR", "F03BL", "F67YLBL", "F47RD", "F42OR", "F38BL", "F48SL", "F51RD", "F71BL",  "F69OR", "F14GR","F65ORWH","W54ORYL", "W46ORYL", "W09RDOR", "W13YLSL",  "W58BLWH","W08RDOR", "W28ORYL", "W49RDYL", "W40GROR", "W56YLBL", "W70RDYL","W01RDYL")
phenotype <- c(rep("R",12), rep("NR", 12))
...
**However, this is not what we are seeing here. Instead, we see a pretty tight clustering of mean and variance (lower dispersion), but with *higher* dispersion in intermediate and high mean genes!** It also appears more curvilinear as a result.
...
```
Heuristically, they all appear to do well, but this was a nice dataset to begin with. Let's just stick with the parametric VST.

```{r}
#save(vsdata1, file = "Pdominula_VarStab.RData")
write.csv(file = "Pdominula_VarStab.csv", assay(vsdata1), row.names = T, quote = F)
```
...
pheatmap(pwCorr)
```
There isn't the greatest clustering, although generally the workers (non-Rs) cluster together (except 2 that cluster with the foundresses), and the foundresses (Rs) cluster together (except 2 that clusters with the workers).
...
DEGenumerator <- function(dds) {
  for (i in 1:length(lfc.thresholds)) {
    ...
    ## Put in a dataframe:
    LFC.df <- rbind(LFC.df, c(lfc.thresholds[i], linear.changes[i], total, up, down, "Pdominula"))
  }
...
plotCounts(dds, gene="gene-LOC107064221", intgroup="phenotype")
plotCounts(dds, gene="gene-LOC107064343", intgroup="phenotype")
plotCounts(dds, gene="gene-LOC107067260", intgroup="phenotype")
```
