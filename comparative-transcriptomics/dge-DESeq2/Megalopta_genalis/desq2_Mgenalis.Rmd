---
title: "DESeq2 Analyses: <i>Megalopta genalis</i>"
author: "KS Geist"
...
## Import data
hymn <- read.table("Mgenalis_merged_gene_counts.txt", header = T, sep = "\t")
...
names(hymn)
colnames(hymn) <- c("SRR3948565", "SRR3948538", "SRR3948545", "SRR3948582", "SRR3948555", "SRR3948573", "SRR3948561", "SRR3948557", "SRR3948523", "SRR3948546", "SRR3948522", "SRR3948569", "SRR3948553", "SRR3948559", "SRR3948567", "SRR3948524", "SRR3948525", "SRR3948578", "SRR3948571", "SRR3948574", "SRR3948551", "SRR3948541", "SRR3948543", "SRR3948576", "SRR3948532", "SRR3948549", "SRR3948530", "SRR3948535", "SRR3948534", "SRR3948580")

## At the moment, we have more samples than desired for repro vs. nonrepro. So, let's just grab the columns we want:
hymn <- hymn[,c("SRR3948522", "SRR3948580", "SRR3948532", "SRR3948545", "SRR3948559", "SRR3948571", "SRR3948576", "SRR3948523", "SRR3948530", "SRR3948535", "SRR3948538", "SRR3948546", "SRR3948551", "SRR3948569")]
#names(hymn)
...
## Read count by phenotype (first 7 are R's, second 7 are NR's)
sum(hymn[,c(1:7)])
sum(hymn[,c(8:14)])
...
```{r}
id <- c("SRR3948522", "SRR3948580", "SRR3948532", "SRR3948545", "SRR3948559", "SRR3948571", "SRR3948576", "SRR3948523", "SRR3948530", "SRR3948535", "SRR3948538", "SRR3948546", "SRR3948551", "SRR3948569")
phenotype <- c(rep("R",7), rep("NR", 7))
...
**However, this is not what we are seeing here. Instead, we see a pretty tight clustering of mean and variance (lower dispersion), but with *higher* dispersion in intermediate .** It also appears more curvilinear as a result.

Hopefully, a variance-stabilizing transformation will help to resolve this issue.
...
plotDispEsts(dds)
```
It looks pretty good. It doesn't cluster as tightly as we would hope, but even the original data (black)  fit pretty well. Post-shrinkage (blue), everything is fitting pretty nicely. There is no pattern to the data nor a huge cloud of points, which is most important.
...
```
Heuristically, they all appear to do well except rld, but there is a lot more dispersion in this dataset than some of the others. Let's just stick with the parametric VST.

```{r}
#save(vsdata1, file = "Mgenalis_VarStab.RData")
write.csv(file = "Mgenalis_VarStab.csv", assay(vsdata1), row.names = T, quote = F)
```
...
```
Ouch. Something is going on! The phenotypes don't cluster distinctly.
...
pheatmap(pwCorr)
```
The clustering is very distinct, but it does not go with our phenotypes. SRR3948538 & SRR3948551 are both workers. SRR3948522 & SRR3948576 are both queens. We see the same clustering in that PCA above. 

Note that in the original paper, brain gene expression was not highly variable (surprising?) and that they saw the least variation in expression in the brains of queens and workers. I think we're just picking up on that here too.
...
DEGenumerator <- function(dds) {
  for (i in 1:length(lfc.thresholds)) {
    ...
    ## Put in a dataframe:
    LFC.df <- rbind(LFC.df, c(lfc.thresholds[i], linear.changes[i], total, up, down, "Mgenalis"))
  }
...
plotCounts(dds, gene="gene-LOC117218713", intgroup="phenotype")
plotCounts(dds, gene="gene-LOC117221120", intgroup="phenotype")
plotCounts(dds, gene="gene-LOC117220716", intgroup="phenotype")
```
